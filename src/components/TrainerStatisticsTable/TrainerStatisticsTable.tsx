...
interface TrainerStatisticsTableProps {
  year?: number
...
const TrainerStatisticsTable = ({
  ...
  breed = 'B',
  amount = 10,
}: TrainerStatisticsTableProps) => {
  const { data } = useSWR<TravsportStats[]>(
    `travsport/?licenseType=${licenseType}&gender=${gender}&homeTrack=${homeTrack}&raceOnTrack=${currentTrack}&typeOfRace=${raceType}&sulkyOrMonte=${sulkyOrMonte}&breed=${breed}&returnNumberOfEntries=${amount}&onlyYouth=false&list=S&year=${year}&chartTypeSearchParam=2&category=1`
  )
...
export default TrainerStatisticsTable
